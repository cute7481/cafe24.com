...
class QreplySQL {
	...
	final static String LIST = "select A.*, B.NICK, B.PIC from R_PIC_REPLY A left outer join R_MEMBER B on A.ID=B.ID "
			+ "where A.PNUM=? order by A.NUM desc";
	//사진 댓글 업데이트
	final static String QREPLY = "insert into R_PIC_REPLY values(NULL, ?, ?, ?, ?, ?, NOW())";
	//댓글 삭제
	...
	//사진별 리플 개수
	final static String QR_TOTAL = "select COUNT(*) as TOTALCOUNT from R_PIC_REPLY where PNUM=?";
